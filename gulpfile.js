...
const jsdocConfig = require(config.documentation.configFile);
...
gulp.task(
  'default',
  gulp.series(['clean', 'html', 'styles', 'scripts', 'images', 'icons'], function(done) {
    browserSync.init({
      server: {
        baseDir: './public/'
      }
...
gulp.task('documentation', function(cb) {
  gulp.src(config.documentation.src, { read: false }).pipe(jsdoc(jsdocConfig, cb));
});

// > Build a production-ready version of your proyect
gulp.task(
  'docs',
  gulp.series(['clean-dist', 'html-dist', 'styles-dist', 'scripts-dist', 'images-dist', 'icons-dist', 'documentation'], function(done) {
